...
export default function FinancialTab() {
  ...
  const API_BASE = "https://healthcare-backend-szmd.onrender.com/api/financial";
  ...
  // Fetch financial data when the year changes
  useEffect(() => {
  ...
  const financialChartOptions = {
    ...
    plotOptions: {
      area: { fillOpacity: 0.25, marker: { enabled: true, radius: 3 } },
    },
    series: [
      { name: "Revenue", data: chartData.map((d) => d.revenue), color: "#3b82f6" },
      { name: "Expenses", data: chartData.map((d) => d.expenses), color: "#ef4444" },
    ],
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-md p-6">
        <h3 className="text-lg font-semibold mb-4">Financial Trends ({selectedYear})</h3>
